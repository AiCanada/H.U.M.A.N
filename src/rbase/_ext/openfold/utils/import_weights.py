...
try:  # Python 3.11+: stop partial enum values becoming method descriptors
...
class ParamType(Enum):
    # partial keeps the callable from becoming a method; enum.member() keeps
    # that value as a data member on 3.13+ (else FutureWarning on every import).
    LinearWeight = _enum_member(
        partial(
            lambda w: w.unsqueeze(-1) if len(w.shape) == 1 else w.transpose(-1, -2)
        )
    )
    LinearWeightMHA = _enum_member(
        partial(lambda w: w.reshape(*w.shape[:-2], -1).transpose(-1, -2))
    )
    LinearMHAOutputWeight = _enum_member(
        partial(
            lambda w: w.reshape(*w.shape[:-3], -1, w.shape[-1]).transpose(-1, -2)
        )
    )
    LinearBiasMHA = _enum_member(partial(lambda w: w.reshape(*w.shape[:-2], -1)))
    LinearWeightOPM = _enum_member(
        partial(
            lambda w: w.reshape(*w.shape[:-3], -1, w.shape[-1]).transpose(-1, -2)
        )
    )
    LinearWeightMultimer = _enum_member(
        partial(
    ...
    LinearBiasMultimer = _enum_member(partial(lambda w: w.reshape(-1)))
    Other = _enum_member(partial(lambda w: w))
