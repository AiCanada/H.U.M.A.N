...
from .primitives import AdaLN, Attention, Linear, LinearNoBias
...
class AttentionPairBias(nn.Module):
    ...
    def __init__(
        ...
        assert dim % no_heads == 0, (
            f"the model dimensionality ({dim}) should be divisible by the "
        ...
        if input_gating:
            ...
            self.output_proj_linear.bias = nn.Parameter(
                torch.ones(dim) * -2.0
            )  # gate values will be ~0.11
        else:
    ...
    def forward(
        ...
        pair_repr: Optional[torch.Tensor] = None,  # (*, N, N, c_z)
        mask: Optional[torch.Tensor] = None,  # (*, N)
        ...
        if self.input_gating:
            a = self.input_proj(
                single_repr, single_proj
            )  # AdaLN(a, s)  shape: (bs, S, n_tokens, c_atom)
        else:
            a = self.input_proj(single_repr)  # LayerNorm
        ...
        # Mask the output
        if mask is not None:
            output = output * mask.unsqueeze(-2).unsqueeze(-1)  # [B, 1, L, 1]
