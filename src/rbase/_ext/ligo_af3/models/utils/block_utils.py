...
from .checkpointing import checkpoint_blocks

def prep_blocks(
    blocks: List[Callable], clear_cache_between_blocks: bool, **kwargs: Any
) -> List[Callable]:
    """Prepare the blocks for the forward pass."""
    prepared_blocks = [partial(block, **kwargs) for block in blocks]
...
def forward_with_checkpointing(
    blocks: List[Callable], args: Tuple[Tensor, ...], blocks_per_ckpt: Optional[int]
) -> Tuple[Tensor, ...]:
