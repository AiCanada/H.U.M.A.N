...
"""Modified model.py from openfold to extract embeddings

See https://github.com/aqlaboratory/openfold/blob/v2.0.0/openfold/model/model.py
...
import rbase._ext.openfold.np.residue_constants as residue_constants
from rbase._ext.openfold.data import data_transforms_multimer
from rbase._ext.openfold.model.embedders import (
    ExtraMSAEmbedder,
    InputEmbedder,
...
from rbase._ext.openfold.model.evoformer import EvoformerStack, ExtraMSAStack
from rbase._ext.openfold.model.heads import AuxiliaryHeads
from rbase._ext.openfold.model.structure_module import StructureModule
from rbase._ext.openfold.model.template import (
    embed_templates_average,
    embed_templates_offload,
)
from rbase._ext.openfold.utils.feats import (
    atom14_to_atom37,
    build_extra_msa_feat,
    pseudo_beta_fn,
)
from rbase._ext.openfold.utils.tensor_utils import (
    add,
    masked_mean,
    tensor_tree_map,
)
...
class AlphaFold(nn.Module):
    ...
    def iteration(self, feats, prevs, _recycle=True):
        ...
        outputs["evo_single"] = s.clone()
        outputs["evo_pair"] = z.clone()
    ...
    def forward(self, batch):
        ...
        outputs["num_recycles"] = torch.tensor(
            num_recycles,
            device=feats["aatype"].device,  # type:ignore
        )

        if "asym_id" in batch:
            outputs["asym_id"] = feats["asym_id"]  # type:ignore
