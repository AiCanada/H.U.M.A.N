...
# Copyright 2022 Jason Yim, Brian L Trippe, Valentin De Bortoli, Emile Mathieu
...
move_to_np = lambda x: x.float().cpu().detach().numpy()
...
class SO3Diffuser:
    def __init__(self, so3_conf):
        ...
        self.alpha = np.inf if so3_conf.alpha < 0 else so3_conf.alpha
    ...
    #     return np.interp(np.random.rand(n_samples), self._cdf[self.t_to_idx(t)], self.discrete_omega)
    ...
    def sample(
        ...
        """
        # x1 = np.random.randn(batch_size,seq_len, 3)
        x1 = self.sample_gauss(
            batch_size, seq_len, num_frames, 3, correlated_noise=correlated_noise
        )  # (F B L 3)
        x1 /= np.linalg.norm(x1, axis=-1, keepdims=True)
        x2 = self.sample_uniform(
            batch_size, seq_len, num_frames, 1, correlated_noise=correlated_noise
        )  # F B L 1

        return x1 * np.interp(x2, self._cdf[self.t_to_idx(t)], self.discrete_omega)

    def sample_gauss(
        self, batch_size, seq_len, num_frames, channel_size=3, correlated_noise=True
    ):
        """Sample correlated Gaussian noise along the frame direction:

        noise[i + 1] = sqrt(1 - b^2) * noise[i] + N(0, b)
        """
        if correlated_noise:
            b = 1 / np.sqrt(1 + self.alpha * self.alpha)
            # a = 0 if self.alpha==0 else 1 / np.sqrt(1 + 1 / (self.alpha * self.alpha))
            a = np.sqrt(1 - b * b)
            noise = []
            noise.append(np.random.randn(batch_size, seq_len, channel_size))
            for i in range(num_frames - 1):
                noise.append(
                    a * noise[i]
                    + np.random.normal(
                        loc=0, scale=b, size=(batch_size, seq_len, channel_size)
                    )
                )
            noise = np.stack(noise, axis=0)
        else:
            noise = np.random.randn(num_frames, batch_size, seq_len, 3)
        return noise  # F B L C

    def sample_uniform(
        self, batch_size, seq_len, num_frames, channel_size=3, correlated_noise=True
    ):
        """Sample correlated uniform noise along the frame direction"""
        if correlated_noise:
            b = 1 / (1 + self.alpha)
            a = 1 - b
            noise = []
            noise.append(np.random.rand(batch_size, seq_len, channel_size))
            for i in range(num_frames - 1):
                noise.append(
                    a * noise[i] + b * np.random.rand(batch_size, seq_len, channel_size)
                )
            noise = np.stack(noise, axis=0)
        else:
            noise = np.random.rand(num_frames, batch_size, seq_len, channel_size)
        return noise  # F B L C

    def sample_ref(self, batch_size, seq_len, num_frames):
        return rearrange(
            self.sample(1, batch_size, seq_len, num_frames),
            "F B ... ->  (B F) ... ",
            F=num_frames,
        )
    ...
    def score(
        ...
        torch_score = self.torch_score(
            torch.tensor(vec), torch.tensor(t)[None], use_cached_score=use_cached_score
        )
    ...
    def forward_marginal(
        ...
        """
        seq_len = rot_0.shape[0] // num_frames
        sampled_rots = self.sample(
            t=t,
            batch_size=1,
            seq_len=seq_len,
        ...
        if num_frames == 1:
            sampled_rots = sampled_rots[None, ...]
        sampled_rots = rearrange(sampled_rots, "F L ... ->  (F L) ... ", F=num_frames)
        rot_score = self.score(
            sampled_rots, t, use_cached_score=use_cached_score
        ).reshape(rot_0.shape)
