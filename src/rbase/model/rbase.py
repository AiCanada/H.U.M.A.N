...
from rbase._ext.openfold.data.data_transforms import pseudo_beta_fn
from tqdm import tqdm
from transformers.models.llama.modeling_llama import LlamaPreTrainedModel

from rbase.data.infer import (
    GenCaseConfig,
    GenDataset,
    GenDatasetConfig,
    PDBConditions,
...
from rbase.data.pretrain_repr import OpenFoldReprLoader
from rbase.env import CachePaths
from rbase.model.decoder import Decoder
from rbase.model.decoder.confdiff.sampler.euler import EulerSampler
from rbase.model.temporal.llama import TRANSFORMERS_VERSION, SinkCache
from rbase.utils import PathLike, get_pylogger
from rbase.utils.misc import download_file
...
class ModelRegistry:
    ...
    def __init__(self, ckpt_dir: PathLike = DEFAULT_PATH.confrover_base) -> None:
    ...
    def get_model_ckpt(self, model_name: str) -> PathLike:
        """Get model ckpt path from model yaml and rbase ckpt dir"""
...
class RBase(
    ...
    def generate(
        ...
        """RBase API to generate conformations or trajectories for single case.

        For batch generation, please use command line tool `rbase generate` instead.
    ...
    def from_config(
        ...
        """Create RBase model from model config (dict/DictConfig) or a yaml file"""
        from rbase.utils.hydra_utils import load_hydra_config
        ...
        assert isinstance(model, cls), f"Model type is not RBase"
        if return_cfg:
    ...
    def from_pretrained(
        ...
        ckpt_dir: PathLike = DEFAULT_PATH.confrover_base,
        seed: int = 42,
        use_deepspeed_evo_attention: bool = False,
        kv_cache_type: str = "offloaded",
    ) -> RBase:
        """Create RBase model from a pretrained model checkpoint"""
